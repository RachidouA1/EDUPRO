...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrfToken($_POST['csrf'] ?? '')) {
        ...
        if (empty($errors)) {
            $db->prepare("UPDATE enseignants SET nom=?,prenom=?,sexe=?,date_naissance=?,telephone=?,email=?,adresse=?,specialite=?,type_contrat=?,salaire_base=?,date_recrutement=?,actif=? WHERE id=?")
               ->execute([
...
<form method="POST" novalidate>
  ...
  <div class="row g-4">
        ...
        <div class="card-body">
          <div class="row g-3">
            <div class="col-md-6">
              ...
              <input type="text" name="nom" class="form-control" value="<?= h($_POST['nom'] ?? $ens['nom']) ?>" required>
            </div>
            <div class="col-md-6">
              <label class="form-label">Prénom(s) *</label>
              <input type="text" name="prenom" class="form-control" value="<?= h($_POST['prenom'] ?? $ens['prenom']) ?>" required>
            </div>
            <div class="col-md-4">
            ...
            <div class="col-md-4">
              ...
              <input type="date" name="date_naissance" class="form-control" value="<?= h($_POST['date_naissance'] ?? $ens['date_naissance']) ?>">
            </div>
            <div class="col-md-4">
              <label class="form-label">Téléphone</label>
              <input type="tel" name="telephone" class="form-control" value="<?= h($_POST['telephone'] ?? $ens['telephone']) ?>">
            </div>
            <div class="col-md-6">
              <label class="form-label">Email</label>
              <input type="email" name="email" class="form-control" value="<?= h($_POST['email'] ?? $ens['email']) ?>">
            </div>
            <div class="col-md-6">
              <label class="form-label">Spécialité</label>
              <input type="text" name="specialite" class="form-control" value="<?= h($_POST['specialite'] ?? $ens['specialite']) ?>">
            </div>
            <div class="col-12">
              <label class="form-label">Adresse</label>
              <textarea name="adresse" class="form-control" rows="2"><?= h($_POST['adresse'] ?? $ens['adresse']) ?></textarea>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="col-lg-4">
      <div class="card">
        <div class="card-header"><i class="fas fa-briefcase me-2 text-primary"></i>Poste & Rémunération</div>
        <div class="card-body">
          <div class="mb-3">
          ...
          <div class="mb-3">
            ...
            <input type="number" name="salaire_base" class="form-control" value="<?= h($_POST['salaire_base'] ?? $ens['salaire_base']) ?>">
          </div>
          <div class="mb-3">
            <label class="form-label">Date de recrutement</label>
            <input type="date" name="date_recrutement" class="form-control" value="<?= h($_POST['date_recrutement'] ?? $ens['date_recrutement']) ?>">
          </div>
          <div class="form-check">
            <input class="form-check-input" type="checkbox" name="actif" id="actif" value="1" <?= ($_POST['actif'] ?? $ens['actif']) ? 'checked' : '' ?>>
            <label class="form-check-label" for="actif">Actif</label>
          </div>
          <div class="alert alert-info mt-3 py-2 fs-sm mb-0">
            <i class="fas fa-id-badge me-2"></i>Matricule : <strong><?= h($ens['matricule']) ?></strong>
          </div>
        </div>
      </div>
    </div>
